"""
cluster_prototypes_hybrid.py
============================
Two-stage hierarchical prototype selection:

  Stage 0 — Downsample the majority "straight-driving" class so the
             action-space clustering is not dominated by near-zero
             steering frames.
...
Outputs (mirrors cluster_prototypes.py layout):
    prototypes_hybrid/
      k{k}/
      ...
      kmeans_hybrid_results.npz
      cluster_summary.txt
      centroid_overview.png

USAGE
-----
    python cluster_prototypes_hybrid.py
"""
...
OUT_ROOT = Path("prototypes_hybrid")
K_VALUES = [2, 4, 6, 8]
N_SAMPLES = 10  # distance-weighted samples saved per cluster
RANDOM_SEED = 42

# --- Downsampling parameters -------------------------------------------------
STRAIGHT_STEER_THRESHOLD = 0.1  # |steer| < this → "straight" frame
STRAIGHT_DROP_RATE = 0.80  # fraction of straight frames to DROP
...
# ─────────────────────────────────────────────────────────────────────────────
# Stage 0 — Downsample straight-driving frames
# ─────────────────────────────────────────────────────────────────────────────
rng = np.random.default_rng(RANDOM_SEED)

is_straight = np.abs(actions[:, 0]) < STRAIGHT_STEER_THRESHOLD
straight_idx = np.where(is_straight)[0]
turning_idx = np.where(~is_straight)[0]

n_straight_keep = int(len(straight_idx) * (1.0 - STRAIGHT_DROP_RATE))
kept_straight = rng.choice(straight_idx, size=n_straight_keep, replace=False)

keep_idx = np.sort(np.concatenate([kept_straight, turning_idx]))

print(f"\nDownsampling straight frames (|steer| < {STRAIGHT_STEER_THRESHOLD}):")
print(
    f"  Straight : {len(straight_idx):,}  →  kept {n_straight_keep:,}  "
    f"(dropped {len(straight_idx) - n_straight_keep:,}, "
    f"{STRAIGHT_DROP_RATE*100:.0f}%)"
)
print(f"  Turning  : {len(turning_idx):,}  (unchanged)")
print(f"  Dataset  : {N:,}  →  {len(keep_idx):,} after downsampling")
...
def plot_3d_clusters(actions_raw, labels, centroids_raw, k, save_path):
    ...
    ax.set_zlabel("Brake")
    ax.set_title(f"K-Means k={k}  |  Action Space (downsampled)")
...
for k in K_VALUES:
    ...
    summary_lines.append(f"k = {k}  (hybrid: action cluster → visual centroid)")
...
# ─────────────────────────────────────────────────────────────────────────────
np.savez_compressed(str(OUT_ROOT / "kmeans_hybrid_results.npz"), **all_results)
print(f"\nSaved combined results → {OUT_ROOT / 'kmeans_hybrid_results.npz'}")
...
fig.suptitle(
    "Hybrid Centroid Prototypes  (action cluster → visual center)", fontsize=12
)
plt.tight_layout()
...
print(f"    kmeans_hybrid_results.npz")
